...
📄 **Tools:** [`src/webmcp/tools.js`](src/webmcp/tools.js), every tool lives in that one file.
...
This is openly a companion planner rather than a retrofit of fantasy.premierleague.com. It is a reference implementation of the pattern, not a fork of the real site.
...
- **Imperative API** for the nine stateful tools: `document.modelContext.registerTool(tool, { signal })` with an `AbortController` for cleanup, `annotations: { readOnlyHint: true }` on the two read-only tools.
...
| `highlight_players` | Outlines players on screen with a short label. Purely visual; it changes nothing about the squad. |
...
The example squad is deliberately imperfect. Ollie Watkins is still in the starting XI having left the league, the bank is tight at £0.8m, and Arsenal is already at the three-per-club limit, so there is somewhere for the agent to go.
