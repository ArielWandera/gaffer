...
MCP server. An assistant asked "should I sell Watkins?" is answering blind about
...
2031. And the architecture is not about football: *the agent reads the draft
...
- **Imperative API** for seven stateful tools: `document.modelContext.registerTool(tool, { signal })`
...
- **No backend.** Vite + React, plain CSS, static build on Vercel.
- **Static FPL snapshot** dated 3 September 2026 (651 players), because the live API
  sends no CORS headers. The current gameweek is derived from the fixture list
  rather than hardcoded. A serverless proxy and real-team import are the named next step.
- `scripts/check.mjs` drives the real reducer and the real `registerTools` against a
  fake `document.modelContext`, covering the club-limit block and recovery, the
  free-transfer → points-hit tool swap, the wildcard vanishing, `readOnlyHint`,
  AbortController teardown, and graceful no-op without WebMCP.
