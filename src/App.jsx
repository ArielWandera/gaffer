...
export default function App() {
  ...
  const [mcp, setMcp] = useState({ ok: false, count: 0, names: [] });
  ...
  useEffect(() => {
    const { supported, names, dispose } = registerTools(state, dispatch);
    setMcp({ ok: supported, count: names.length, names });
    return dispose;
  }, [state]);
